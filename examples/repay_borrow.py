...
print("~" * 100)
print("Processing repay_borrow transaction")
print("~" * 100)
...
txn = client.prepare_borrow_transactions(symbol, int(1000), sender)
txn.sign_with_private_key(sender, key)
txn.submit(client.algod, wait=True)
txn = client.prepare_repay_borrow_transactions(symbol, int(100), sender)
txn.sign_with_private_key(sender, key)
txn.submit(client.algod, wait=True)
