...
print("~" * 100)
print("Processing sync_vault transaction")
print("~" * 100)
...
storage_account = client.manager.get_storage_address(sender)
txn = prepare_payment_transaction(
    sender, suggested_params, storage_account, int(0.10 * asset_balance)
)
txn.sign_with_private_key(sender, key)
txn.submit(client.algod, wait=True)
txn = client.prepare_sync_vault_transactions(sender)
txn.sign_with_private_key(sender, key)
txn.submit(client.algod, wait=True)
