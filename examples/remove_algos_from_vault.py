...
print("~" * 100)
print("Processing remove_algos_from_vault transaction")
print("~" * 100)
print("Processing transaction for asset = %s" % (symbol))
txn = client.prepare_supply_algos_to_vault_transactions(
    int(asset_balance * 0.10), sender
)
txn.sign_with_private_key(sender, key)
txn.submit(client.algod, wait=True)
user_active_collateral = client.get_user_state(sender)[symbol]["active_collateral_bank"]
txn = client.prepare_remove_algos_from_vault_transactions(int(28907346 // 5000), sender)
txn.sign_with_private_key(sender, key)
txn.submit(client.algod, wait=True)
