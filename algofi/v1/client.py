...
from .remove_collateral_underlying import (
    prepare_remove_collateral_underlying_transactions,
)
...
from .staking import (
    prepare_staking_contract_optin_transactions,
...
class Client:
    def __init__(
        ...
        self.max_atomic_opt_in_ordered_symbols = get_ordered_symbols(
            self.chain, max_atomic_opt_in=True
        )

        # manager info
        self.manager = Manager(
            self.indexer, self.historical_indexer, get_manager_app_id(self.chain)
        )
        ...
        self.markets = {
            symbol: Market(
                ...
                get_market_app_id(self.chain, symbol),
            )
            for symbol in self.max_ordered_symbols
        }
        ...
        self.staking_contracts = {
            name: StakingContract(
                self.indexer, self.historical_indexer, self.staking_contract_info[name]
            )
            for name in self.staking_contract_info.keys()
        }
    ...
    def get_storage_state(self, storage_address=None, block=None, include_manager=True):
        ...
        if include_manager:
            result["manager"] = self.manager.get_storage_state(
                storage_address, block=block
            )
        ...
        for symbol in active_markets:
            result[symbol] = self.markets[symbol].get_storage_state(
                storage_address, block=block
            )
        return result
    ...
    def get_active_markets(self):
        ...
        return dict(
            filter(
                lambda elem: elem[0] in self.active_ordered_symbols,
                self.markets.items(),
            )
        )
    ...
    def get_max_atomic_opt_in_market_app_ids(self):
        ...
        """
        return [
            self.markets[symbol].get_market_app_id()
            for symbol in self.max_atomic_opt_in_ordered_symbols
        ]
    ...
    def get_active_assets(self):
        ...
        """
        return {
            symbol: market.get_asset()
            for symbol, market in self.get_active_markets().items()
        }
    ...
    def get_active_asset_ids(self):
        ...
        return [
            ...
            for asset in self.get_active_assets().values()
        ]
    ...
    def get_active_bank_asset_ids(self):
        ...
        """
        return [
            asset.get_bank_asset_id() for asset in self.get_active_assets().values()
        ]
    ...
    def get_raw_prices(self, update=True):
        ...
        """
        return {
            symbol: market.get_asset().get_raw_price(update=update)
            for symbol, market in self.get_active_markets().items()
        }
    ...
    def get_prices(self, update=True):
        ...
        """
        return {
            symbol: market.get_asset().get_price(update=update)
            for symbol, market in self.get_active_markets().items()
        }
    ...
    def get_storage_accounts(self, staking_contract_name=None, verbose=False):
        ...
        next_page = ""
        accounts = []
        user_address = base64.b64encode(
            bytes(manager_strings.user_address, "utf-8")
        ).decode("utf-8")

        if staking_contract_name is None:
            app_id = list(self.get_active_markets().values())[0].get_market_app_id()
        else:
            app_id = self.get_staking_contract(
    ...
    def get_active_oracle_app_ids(self):
        ...
        """
        return [
            market.get_asset().get_oracle_app_id()
            for market in self.get_active_markets().values()
        ]
    ...
    def get_active_market_app_ids(self):
        ...
        """
        return [
            market.get_market_app_id() for market in self.get_active_markets().values()
        ]
    ...
    def get_active_market_addresses(self):
        ...
        """
        return [
            market.get_market_address() for market in self.get_active_markets().values()
        ]
    ...
    def prepare_optin_transactions(self, storage_address, address=None):
        ...
        if not address:
            address = self.user_address
        return prepare_manager_app_optin_transactions(
    ...
    def prepare_add_collateral_transactions(self, symbol, amount, address=None):
        ...
        return prepare_add_collateral_transactions(
            address,
            self.get_default_params(),
    ...
    def prepare_borrow_transactions(self, symbol, amount, address=None):
        ...
        if not address:
            address = self.user_address
        market = self.get_market(symbol)
        return prepare_borrow_transactions(
            address,
            self.get_default_params(),
            self.manager.get_storage_address(address),
            amount,
    ...
    def prepare_burn_transactions(self, symbol, amount, address=None):
        ...
        if not address:
            address = self.user_address
        market = self.get_market(symbol)
        return prepare_burn_transactions(
            address,
            self.get_default_params(),
            self.manager.get_storage_address(address),
            amount,
    ...
    def prepare_claim_rewards_transactions(self, address=None):
        ...
        if not address:
            address = self.user_address
        return prepare_claim_rewards_transactions(
            address,
            self.get_default_params(),
            self.manager.get_storage_address(address),
            self.manager.get_manager_app_id(),
    ...
    def prepare_liquidate_transactions(
        ...
        return prepare_liquidate_transactions(
            address,
            params,
            self.manager.get_storage_address(address),
            target_storage_address,
            ...
            borrow_market.get_asset().get_underlying_asset_id()
            if borrow_symbol != "ALGO"
            else None,
    ...
    def prepare_mint_transactions(self, symbol, amount, address=None):
        ...
        if not address:
            address = self.user_address
        market = self.get_market(symbol)
        return prepare_mint_transactions(
            address,
            self.get_default_params(),
            self.manager.get_storage_address(address),
            amount,
    ...
    def prepare_mint_to_collateral_transactions(self, symbol, amount, address=None):
        ...
        if not address:
            address = self.user_address
        market = self.get_market(symbol)
        return prepare_mint_to_collateral_transactions(
            address,
            self.get_default_params(),
            self.manager.get_storage_address(address),
            amount,
    ...
    def prepare_remove_collateral_transactions(self, symbol, amount, address=None):
        ...
        if not address:
            address = self.user_address
        market = self.get_market(symbol)
        return prepare_remove_collateral_transactions(
            address,
            self.get_default_params(),
            self.manager.get_storage_address(address),
            amount,
    ...
    def prepare_remove_collateral_underlying_transactions(
        self, symbol, amount, address=None
    ):
        ...
        if not address:
            address = self.user_address
        market = self.get_market(symbol)
        return prepare_remove_collateral_underlying_transactions(
            address,
            self.get_default_params(),
            self.manager.get_storage_address(address),
            amount,
    ...
    def prepare_repay_borrow_transactions(self, symbol, amount, address=None):
        ...
        if not address:
            address = self.user_address
        market = self.get_market(symbol)
        return prepare_repay_borrow_transactions(
            address,
            self.get_default_params(),
            self.manager.get_storage_address(address),
            amount,
    ...
    def prepare_staking_contract_optin_transactions(
        self, staking_contract_name, storage_address, address=None
    ):
        ...
        return prepare_staking_contract_optin_transactions(
    ...
    def prepare_stake_transactions(self, staking_contract_name, amount, address=None):
        ...
        return prepare_stake_transactions(
            address,
            self.get_default_params(),
    ...
    def prepare_unstake_transactions(self, staking_contract_name, amount, address=None):
        ...
        staking_contract = self.get_staking_contract(staking_contract_name)
        asset_id = staking_contract.get_asset().get_underlying_asset_id()
        return prepare_unstake_transactions(
            address,
            self.get_default_params(),
            staking_contract.get_storage_address(address),
            amount,
    ...
    def prepare_claim_staking_rewards_transactions(
        self, staking_contract_name, address=None
    ):
        ...
        staking_contract = self.get_staking_contract(staking_contract_name)
        asset_id = staking_contract.get_asset().get_underlying_asset_id()
        return prepare_claim_staking_rewards_transactions(
            address,
            self.get_default_params(),
            staking_contract.get_storage_address(address),
            staking_contract.get_manager_app_id(),
    ...
    def prepare_supply_algos_to_vault_transactions(self, amount, address=None):
        ...
        return prepare_supply_algos_to_vault_transactions(
            address,
            self.get_default_params(),
            self.manager.get_storage_address(address),
            amount,
    ...
    def prepare_remove_algos_from_vault_transactions(self, amount, address=None):
        ...
        if not address:
            address = self.user_address
        market = self.get_market("vALGO")
        return prepare_remove_algos_from_vault_transactions(
            address,
            self.get_default_params(),
            self.manager.get_storage_address(address),
            amount,
    ...
    def prepare_sync_vault_transactions(self, address=None):
        ...
        if not address:
            address = self.user_address
        market = self.get_market("vALGO")
        return prepare_sync_vault_transactions(
            address,
            self.get_default_params(),
            self.manager.get_storage_address(address),
            self.manager.get_manager_app_id(),
    ...
    def prepare_send_governance_commitment_transactions(
        self,
        governance_address,
        commitment_amount,
        address=None,
        beneficiary=None,
        check_vault_balance=True,
    ):
        ...
        vault_account_algo_balance = self.get_user_balance(
            asset_id=1, address=vault_address
        )
        if commitment_amount > vault_account_algo_balance:
            raise Exception(
                "Commitment amount of "
                + str(commitment_amount)
                + " microAlgos exceeds ALGO balance of vault with address "
                + vault_address
                + " (balance: "
                + str(vault_account_algo_balance)
                + ")"
            )
        ...
        if beneficiary:
            note = b'af/gov1:j{"com":%s,"bnf":"%s"}' % (
                bytes(str(commitment_amount), "utf-8"),
                bytes(beneficiary, "utf-8"),
            )
        else:
            note = b'af/gov1:j{"com":%s}' % (bytes(str(commitment_amount), "utf-8"))

        return prepare_send_governance_transactions(
            address,
            self.get_default_params(),
            self.manager.get_storage_address(address),
            governance_address,
    ...
    def prepare_send_governance_vote_transactions(
        self, governance_address, note, address=None
    ):
    ...
    def prepare_send_keyreg_online_transactions(
        ...
        if not address:
            address = self.user_address
        return prepare_send_keyreg_online_transactions(
            address,
            self.get_default_params(),
            self.manager.get_storage_address(address),
            vote_pk,
    ...
    def prepare_send_keyreg_offline_transactions(self, address=None):
        ...
        if not address:
            address = self.user_address
        return prepare_send_keyreg_offline_transactions(
            address,
            self.get_default_params(),
            self.manager.get_storage_address(address),
            self.manager.get_manager_app_id(),
...
class AlgofiTestnetClient(Client):
    def __init__(self, algod_client=None, indexer_client=None, user_address=None):
        ...
        super().__init__(
            ...
            historical_indexer_client=historical_indexer_client,
